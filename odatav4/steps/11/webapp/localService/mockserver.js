sap.ui.define(["sap/ui/model/json/JSONModel", "sap/base/Log"], function (JSONModel, Log) {
  ...
  let sinon;
  let sandbox;
  let users; // The array that holds the cached user data
  let metadata; // The string that holds the cached mock service metadata
   ...
   */
  function getUserDataFromRequestBody(body) {
   ...
   */
  function getUserKeyFromUrl(url) {
  ...
  function getSuccessResponse(responseBody) {
    return [200, {
  ...
  function readData() {
    ...
    const mockDataPromise = new Promise((resolve, reject) => {
      ...
      mockDataModel.attachRequestCompleted(function (event) {
        // 404 is not an error for JSONModel so we need to handle it here
        if (event.getParameter("errorobject") && event.getParameter("errorobject").statusCode === 404) {
          const error = "resource '" + resourcePath + "' not found";
          Log.error(error, logComponent);
          reject(new Error(error));
        }
        users = this.getData().value;
        resolve();
      });
   ...
   */
  function applySkipTop(xhr, resultSet) {
   ...
   * Does NOT change the given result set but returns a new array.
   */
  function applySort(xhr, resultSet) {
   ...
   * Does NOT change the given result set but returns a new array.
   */
  function applyFilter(xhr, resultSet) {
   ...
   */
  function handleGetUserRequests(xhr, _bCount) {
    let count;
    let expand;
    let expand2;
    let index;
    ...
    if (key) {
      ...
      if (/People\(.+\)\/Friends/.test(xhr.url)) {
        ...
        response = {
          value: createFriendsArray(users[index].Friends, select)
        };
      } else {
        // specific user was requested
        response = getUserObject(index, select, expand, subSelects);
      }
      if (index > -1) {
    ...
    result.forEach(user => {
      ...
      finalResponse.value.push(getUserObject(userIndex, select, expand, subSelects));
    });
   ...
   */
  function getUserByIndex(index, properties) {
    const helper = {
      UserName: ""
    };
   ...
   */
  function getUserObject(index, select, expand, subSelects) {
    let bestFriend;
    let friendIndex;
    let friends;
    let object;
      ...
      for (i = 0; i < expand.length; i++) {
        switch (expand[i]) {
          case "Friends":
            ...
            object.Friends = createFriendsArray(friends, subSelects[i]);
            break;
          case "BestFriend":
            bestFriend = user.BestFriend;
            friendIndex = findUserIndex(bestFriend || "");
            object.BestFriend = getUserByIndex(friendIndex, subSelects[i]);
            break;
          default:
            break;
        }
   ...
   */
  function createFriendsArray(friends, subSelects) {
    let array = [];
    if (friends) {
   ...
   */
  function handlePatchUserRequests(xhr) {
    ...
    if (Object.prototype.hasOwnProperty.call(changes, "UserName") && changes.UserName !== key && !isUnique(changes.UserName)) {
      // Error
   ...
   */
  function handleDeleteUserRequests(xhr) {
    const key = getUserKeyFromUrl(xhr.url);
   ...
   */
  function handlePostUserRequests(xhr) {
    const user = getUserDataFromRequestBody(xhr.requestBody || "");

    // Check if that user already exists
    if (isUnique(user.UserName)) {
      users.push(user);
      let responseBody = '{"@odata.context": "' + getBaseUrl(xhr.url) + '$metadata#People/$entity",';
   ...
   */
  function handleDirectRequest(xhr) {
    let response2;
    switch (xhr.method) {
      case "GET":
   ...
   */
  function handleBatchRequest(xhr) {
    let responseBody = "";
   ...
   */
  function handleAllRequests(xhr) {
    let response2;

    // Log the request
    Log.info("Mockserver: Received " + xhr.method + " request to URL " + xhr.url, (xhr.requestBody ? "Request body is:\n" + xhr.requestBody : "No request body.") + "\n", logComponent);
    ...
    // Log the response
    if (response2) {
      Log.info("Mockserver: Sent response with return code " + response2[0], "Response headers: " + JSON.stringify(response2[1]) + "\n\nResponse body:\n" + (response2[2] || "") + "\n", logComponent);
    }
  }
  return {
    /**
      ...
      return new Promise((resolve, reject) => {
        sap.ui.require(["sap/ui/thirdparty/sinon"], lazySinon => {
          ...
          readData().then(() => {
            ...
            sinon.FakeXMLHttpRequest.addFilter((_sMethod, url) => {
